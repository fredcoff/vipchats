...
Stop SMS link: http://51.15.193.57:5000/get/cancel/${phoneNumber}`;
    ...
    this.SendReminderMessage = function(phoneNumber) {
        console.log("Send Reminder Message to: " + phoneNumber);
        let reminderMessage = 'We remind you checkIn of your booking.';
        if (SendPromotion){
            SendMessage(phoneNumber, reminderMessage, MessageType.REMINDER);
        }
    };
...
Cancelar SMS: http://51.15.193.57:5000/get/subscribe/${phoneNumber}`;
    ...
    this.SendSms = function(phoneNumber,textMessage, msg_type, hotelid, callback) {
        var date=getDateTime();
        console.log("Sending SMS : " + date + " phone: " + phoneNumber);
        SendMessage(phoneNumber, textMessage, msg_type, hotelid,  callback);
    };

     function SendMessage(phoneNumber, textMessage, msg_type, hotelid, callback) {
        if(callback==null) callback = function(err,message){};
        if(msg_type==null) msg_type = 0;
        //==================================
         //console.log(SendMsg);
         logMessage(phoneNumber, textMessage, msg_type, hotelid);
         if(SendMsg) {
    ...
    function logMessage(phoneNumber, textMessage, msg_type, hotelid){
        // 649806826
        let msg = new Message();
